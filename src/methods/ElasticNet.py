...
def ElasticNet(Train, ExoTrain, ExoTest, cv=5, window=False, lags = False, scaled_coeff=False):
    ...
    Ind.append("L1-L2 Ratio")
    Koeff= pd.DataFrame(index=Ind,columns=[methodname.ElasticNet])
    #Koeff= pd.Series(index=Ind,name="ElasticNet",dtype=float)
    
    pipe = Pipeline([
        ('scale', preprocessing.StandardScaler()),
        ('clf', ElasticNetCV(l1_ratio=l1_ratios,alphas=alphas,cv=cv, max_iter=10000, selection="cyclic",fit_intercept=True,n_jobs=-1))])
    

 
    ElasticNet = pipe.fit(ExoTrain,Train.values.ravel())
    

    Fit =  pd.Series(ElasticNet.predict(ExoTrain).ravel(),index=Train.index)
    Pred = pd.Series(ElasticNet.predict(ExoTest).ravel(),index=ExoTest.index)
    
    
    if scaled_coeff==False:
        coeff = ElasticNet.named_steps['clf'].coef_ /ElasticNet.named_steps['scale'].scale_
        intercept =  ElasticNet.named_steps['clf'].intercept_ -  sum(ElasticNet.named_steps['clf'].coef_ * ElasticNet.named_steps['scale'].mean_ / ElasticNet.named_steps['scale'].scale_)
    
        coeff = np.append(coeff,intercept)
        coeff = np.append(coeff,ElasticNet.named_steps['clf'].alpha_)
        coeff = np.append(coeff,ElasticNet.named_steps['clf'].l1_ratio_)
        Koeff[methodname.ElasticNet]= pd.Series(coeff,index=Ind)

    else:
        coeff = ElasticNet.named_steps['clf'].coef_ 
        intercept =  ElasticNet.named_steps['clf'].intercept_
